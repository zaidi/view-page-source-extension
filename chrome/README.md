# View Page Source - Chrome Extension

A Chrome browser extension that allows you to quickly view any webpage's HTML source code, detect technologies, and get comprehensive SEO analysis on [View Page Source](https://www.view-page-source.com).
...
1. **Download the Extension**
   ...
   - Navigate to the `browser-plugin/chrome/` directory

2. **Open Chrome Extensions Page**
   - Open Chrome and go to `chrome://extensions/`
   - Or open the browser menu, then More Tools, then Extensions
...
4. **Load the Extension**
   - Click "Load unpacked"
   - Select the `browser-plugin/chrome/` folder
...
- SEO Check, Social Preview, and Extract HTML can each be given their own shortcut at `chrome://extensions/shortcuts`
...
Your settings are saved automatically and synced across devices (if signed into Chrome). Your **recent-history** list is stored locally on this device only.
...
- **activeTab** - Access the current tab's URL only when you invoke the extension
...
3. Click the edit (pencil) icon next to any command and set your preferred shortcut

## Troubleshooting

### Extension Icon is Greyed Out
The current page is not supported (e.g., `chrome://` pages). Try navigating to an HTTP or HTTPS website.
...
├── background.js           # Service worker (menus, shortcuts, Quick Look, badge)
├── popup/
...
├── _locales/
│   └── en/messages.json   # Localized strings (English; more locales drop in here)
...
4. Zip the `chrome/` folder for upload to Chrome Web Store
