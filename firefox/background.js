 ...
 * browserAction.onClicked fire ("instant"); setting it restores the popup.
 */
...
browser.runtime.onInstalled.addListener(applyToolbarAction);
browser.runtime.onStartup.addListener(applyToolbarAction);
...
function setStatusBadge(tabId, httpCode) {
  ...
  try {
    ...
    browser.browserAction.setBadgeText({ tabId, text: String(httpCode) });
  } catch (error) {
    // ignore
  }
}
...
// Run analysis in the background so host permissions apply and the request
// isn't subject to the site's CORS policy. Returning a Promise sends its
// resolved value back as the response (the WebExtension idiom).
browser.runtime.onMessage.addListener((msg) => {
  if (msg && msg.type === 'quick-look' && msg.url) {
    return quickLook(msg.url);
  }
});
...
/**
 * Extracts the <title> from raw HTML
 * @param {string} html
 * @returns {string}
 */
